...
** Editing May 12 to edit graphs and re-order countries 
** Editing May 30 to edit order of tables to match graphs. 
...
** Graph prop 70+ by country (check y axis to make sure all in agreement. Also, year range)
	...
	gsort - ref prop_over70
	gen order =_n
	replace order=. if ref!=2015
	gsort country_name -ref 
	...
	twoway scatter prop_over70 reference_date if reference_date>=1975, xtitle("Year") by(order) ylabel(,angle(0)) msize(large) legend(label(1 "Pop 70+") region(lstyle(none)))   xscale(range(1975(20)2015)) xlabel(1975(20)2015) ytitle("Proportion 70+") mcolor(black)  scheme( s1mono  )     graphregion(fcolor(white))
	
	graph export "Graphs/prop_over70_2015.png", replace height(7000) width(8000)
	rename reference_date_as_of_1_july reference_date
	keep country_name reference_date prop_over70 order  ordernum
	tempfile data1 
	save `data1', replace
	...
	gen ordernum=order 
	outsheet using "Data/orderkey.csv", comma replace 
	
	
	
******************************************
** OTHER MEASURES OF AGING - MAD of proportion 70+ 
******************************************
**calculate MAD
	use `data1', clear
	...
	replace ordernum=21 if country_name=="xMAD"
	order ordernum country_name reference_date prop_over70  
	

	reshape wide  prop_over70 ,i(country_name order* ) j(reference_date)
	sort ordernum 
	outsheet using "Results\measures_of_aging_mad_2015_ordered.csv", comma replace
...
** exporting age specific population 
	...
	save "Data\Prepped\reference_pop_2015.dta", replace 